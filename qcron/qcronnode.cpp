...
match(const QDateTime & dt) const
{
    ...
    QCronIntNode * begin = dynamic_cast<QCronIntNode *>(_what);
    if (begin == NULL)
    {
        qDebug() << "(every type  X/X) czz getinfo-->field:" << _what->_field << " first position not an int ";

        QCronAllNode * allMatchNode = dynamic_cast<QCronAllNode *>(_what);
        if(allMatchNode == NULL)
        {
            qDebug() << "(every type  X/X) czz getinfo-->field:" << _what->_field << " first position not an  '*' Oops not  'int' either!!  ";
            return false;
        }
        else
        {
    ...
    {
        //is an int
        qDebug() << "(every type  X/X) czz getinfo-->field:" << _what->_field->getField() << " first position an int: " << begin->value();
        qDebug() << QString("(every type  X/X) max:%1  min:%2").arg(_what->_field->getMax()).arg(_what->_field->getMin());
        int init_value = begin->value();
        while(init_value <= _what->_field->getMax())
        {
            if(init_value == tu)
                return true;

            init_value += _freq->value();
        }
