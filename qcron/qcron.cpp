...
_checkState()
{
    ...
    //qDebug() << "we are ready to check......state---->";

    if (match(QDateTime::currentDateTime()))
    {
        qDebug() << "czz says ,we are matched  --------- " << QDateTime::currentDateTime();
    ...
    {
        //qDebug() << "we go into --->_is_active:" << _is_active;
        ...
        qDebug() << "czz says ,before we get interval_ms,check  next()" << QDateTime::currentDateTime();

        interval_ms = 1000*5;/*QDateTime::currentDateTime().secsTo(next()) * 1000;*/       //1000*10;

        //qDebug() << "czz------- DateTime now--------------------" << QDateTime::currentDateTime();
        //qDebug() << "czz ------ interval_ms-------check interval:" << interval_ms;
    }
...
next(QDateTime dt)
{
    ...
    while (!match(dt))
    {
        qDebug() << "czz in next fun---dt:" << dt << " doesn't match";
        for (int i = YEAR; i >= 0; --i)
        {
            ...
            {
                qDebug() << "czz  field  check ------ " << dt << (EField)i << "doesn't match";
                int dummy = 1;
